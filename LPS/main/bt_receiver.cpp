            ...
            if (ad_type == 0xFF && ad_len >= 16) {
                ...
                if (adv_data[offset] == (target_id & 0xFF) && 
                    ...
                    if (is_target) {
                        ...
                        uint32_t rcv_prep_time = (adv_data[offset+15] << 24) |
...
static void IRAM_ATTR timer_timeout_cb(void *arg) {
    ...
    switch (cmd) {
        case 0x01: Player::getInstance().play(); break;
        case 0x02: Player::getInstance().pause(); break;
        case 0x03: Player::getInstance().reset(); break;
        case 0x04: Player::getInstance().release(); break;
        case 0x05: Player::getInstance().load(); break;
        case 0x06: Player::getInstance().test(test_data[0], test_data[1], test_data[2]); break;
        // --- Cancel Command ---
        case 0x07: {
                ...
                if (now < (window_start_time + s_config.sync_window_us)) {
                    ...
                    if (count > 0) {
                        ...
                        if (wait_us > 500) {
                            ...
                            ESP_LOGI(TAG, "CMD 0x%02X Locked! Slot %d Used. Delay: %lld us (samples: %d)", 
                                     current_cmd, current_cmd_id, wait_us, count);
                        } else {
