...
The internal buffer size is fixed at compile-time based on `LD_BOARD_WS2812B_MAX_PIXEL_NUM`.
...
|                         |  Size = 3 * LD_BOARD_WS2812B_MAX_PIXEL_NUM |
...
* **Constraints:** `pixels` must not exceed `LD_BOARD_WS2812B_MAX_PIXEL_NUM`.
